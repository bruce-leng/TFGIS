...
// 追加(07.07.21)
BOOL CRulerSplitterWnd::IsShow()
{
	return m_bRulersVisible;
}
...
{ 
	...
	{
		rulerRect.right = m_DocSize.cx;
	}
	else //(m_rulerType==RT_VERTICAL)
	{
		rulerRect.bottom = m_DocSize.cy;
	}
...
void CRulerView::DrawTicker(CDC* pDC, CRect rulerRect, int nFactor, int nBegin, BOOL bShowPos)
{
	...
	//double dbFactor  = nFactor*m_fZoomFactor;

	int nTick1;
	for (int i=1; i<=nSize/nFactor; i++)	//将nFactor修改成nTick,2005.11.24
	{
		char buffer[100];
		sprintf(buffer, "%d", i*nFactor/100);

		if (m_rulerType==RT_HORIZONTAL)
		{
			nTick1 = int(nFactor*i*m_fZoomFactor-m_scrollPos.x);
			pDC->PatBlt(nTick1, rulerRect.top+nBegin, 1, rulerRect.bottom, BLACKNESS);
			if (bShowPos)
			{
				pDC->TextOut(nTick1, rulerRect.top, CString(buffer));
			}
		}
		else //(m_rulerType==RT_VERTICAL)
		{
			nTick1 = int(nFactor*i*m_fZoomFactor-m_scrollPos.y);
			pDC->PatBlt(rulerRect.left+nBegin, nTick1, rulerRect.right, 1, BLACKNESS);
			if (bShowPos)
			{
				pDC->TextOut(rulerRect.left, nTick1, CString(buffer));
			}
		}
	}
}
...
void CRulerView::DrawCursorPos(CPoint NewPos)
{
	...
	if (m_rulerType==RT_HORIZONTAL)
	{
		...
		pDC->PatBlt(m_lastPos.x-m_scrollPos.x, clientRect.top, 1, clientRect.bottom, DSTINVERT);
	...
	{
		// erase the previous position
		pDC->PatBlt(clientRect.left, m_lastPos.y-m_scrollPos.y, clientRect.right, 1, DSTINVERT);
		// draw the new position
