...
{
	// 设置映射坐标
	if (pFace->v1()->z()<pFace->v2()->z() || pFace->v1()->z()<pFace->v3()->z())
		pCoords[0].y = 0;
	else
		pCoords[0].y = 1;

	if (pFace->v2()->z()<pFace->v1()->z() || pFace->v2()->z()<pFace->v3()->z())
		pCoords[1].y = 0;
	else
		pCoords[1].y = 1;

	if (pFace->v3()->z()<pFace->v1()->z() || pFace->v3()->z()<pFace->v2()->z())
		pCoords[2].y = 0;
	else
		pCoords[2].y = 1;
	...
	for (int k=0; k<3; k++)
	{
		if (fabs(pFace->v(k)->x()-arc[0][0])<0.001
			&& fabs(pFace->v(k)->y()-arc[0][1])<0.001)
		{
			pCoords[k].x = dbSum;
		}
	}
	for (int i=1; i<21; i++)
	{
		dbSum += sqrt((arc[i-1][0]-arc[i][0])*(arc[i-1][0]-arc[i][0])
					+(arc[i-1][1]-arc[i][1])*(arc[i-1][1]-arc[i][1]));
		for (int k=0; k<3; k++)
		{
			if (fabs(pFace->v(k)->x()-arc[i][0])<0.001
				&& fabs(pFace->v(k)->y()-arc[i][1])<0.001)
			{
				pCoords[k].x = dbSum;
			}
		}
...
void FindTexCoordRock(CFace3d* pFace, PPOINT2DF pCoords)
{
	...
	fMaxX = fMinX = pFace->v1()->x();
	fMaxY = fMinY = pFace->v1()->y();
	for (int i=1; i<3; i++)
	{
		if (fMaxX<pFace->v(i)->x())	fMaxX=pFace->v(i)->x();
		if (fMinX>pFace->v(i)->x())	fMinX=pFace->v(i)->x();
		if (fMaxY<pFace->v(i)->y())	fMaxY=pFace->v(i)->y();
		if (fMinY>pFace->v(i)->y())	fMinY=pFace->v(i)->y();
	}

	for (i=0; i<3; i++)
	{
		pCoords[i].x = (pFace->v(i)->x()-fMinX)/(fMaxX-fMinX)/30;
		pCoords[i].y = (pFace->v(i)->y()-fMinY)/(fMaxY-fMinY)/30;
	}
}
...
//	m_pTextureCoordinateIndex = NULL;
	...
	m_nMeshType = SIDE_FACE;

	
	m_lIdLow = m_lIdHigh = LONG_MAX;

	m_fLockOffset[0] = m_fLockOffset[1] = 0;
	...
	{
		{
			...
			for(int j=0;j<3;j++)
			{
				...
				if(m_IndexTexture != -1)// && g_bDrawText
				{
//////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////
//此处修改非常重要。
//原始纹理坐标采用了坐标索引的方式，用于VMRL语言的规则。
//由于VRML语言的纹理坐标采用序列式方式存储
//所以，此处可以看到他们使用了内部存储的方式
//但是，我们使用了相对坐标的方式，所以在这里是使用的是直接坐标方式
					...
					{
					case MESH_FACE:
						glTexCoord2f(pFace->v(j)->x()+m_fLockOffset[0], pFace->v(j)->y()+m_fLockOffset[1]);	
						break;
					case MESH_STRUCT:
					case MESH_ROCK:
						glTexCoord2f(pFace->v(j)->z()+m_fLockOffset[0], pFace->v(j)->y()+m_fLockOffset[1]);	
						break;
						break;
					case MESH_ARC:
						break;
...
//////////////////////////////////////////////////////////////////////////
			...
			// 加入隧道边墙的纹理贴图(09.05.09)
//			if (GetMeshType()==MESH_ARC)
//			{
//				POINT2DF pt[3];
//				FindTexCoord(pFace, pt);
//				for (int m=0; m<3; m++)
//				{	glTexCoord2f(pt[m].x+m_fLockOffset[0],pt[m].y+m_fLockOffset[1]);
//					::glVertex3f(pFace->v(m)->x(),pFace->v(m)->y(),pFace->v(m)->z());
//				}
//			}
//			if (m_IndexTexture==8)
//			{
//				POINT2DF pt[3];
//				FindTexCoordRock(pFace,pt);
//				for (int m=0; m<3; m++)
//				{	glTexCoord2f(pt[m].x+m_fLockOffset[0],pt[m].y+m_fLockOffset[1]);
//					::glVertex3f(pFace->v(m)->x(),pFace->v(m)->y(),pFace->v(m)->z());
//				}
