...
int CTexture::ReadFile(char *filename)
{
	...
	image.Resample(m_Width, m_Height);
	image.Flip();
	if (image.GetBpp()!=24)
	...
	::GetTempFileName(Path, TXTPREFIX, 0, Path);
	...
	TRY
	{
		file.Write(image.GetBits(), m_Width * m_Height * m_Depth / 8);
	}
	...
	m_pData = (unsigned char*)m_pMemFile->Open();
