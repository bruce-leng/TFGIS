// DlgFaceInfo.cpp : implementation file
//
...
CDlgFaceInfo::CDlgFaceInfo(CWnd* pParent /*=NULL*/)
	: CDialog(CDlgFaceInfo::IDD, pParent)
{
	// 掌子面数量
	m_nFaceNum = 0;
	m_nFaceIdx = 0;
...
CDlgFaceInfo::~CDlgFaceInfo()
{
	if (m_pFacePhoto != NULL)
		delete m_pFacePhoto;
}

void CDlgFaceInfo::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CDlgFaceInfo)
	...
	DDX_Control(pDX, IDC_EDT_REC_STAFF, m_edtRecStaff);
	DDX_Control(pDX, IDC_EDT_REC_REVIEW, m_edtRecReview);
	...
	DDX_Text(pDX, IDC_EDT_REC_REVIEW, m_strRecReview);
	DDV_MaxChars(pDX, m_strRecReview, 20);
	DDX_Text(pDX, IDC_EDT_REC_STAFF, m_strRecStaff);
	DDV_MaxChars(pDX, m_strRecStaff, 20);
...
BEGIN_MESSAGE_MAP(CDlgFaceInfo, CDialog)
	//{{AFX_MSG_MAP(CDlgFaceInfo)
...
/////////////////////////////////////////////////////////////////////////////
// CDlgFaceInfo message handlers
BOOL CDlgFaceInfo::OnInitDialog() 
{
...
void CDlgFaceInfo::OnRecordFirst() 
{
...
void CDlgFaceInfo::OnRecordPrev() 
{
	// 如果处于修改状态，保存原来显示的FaceInfo
	if (!m_btnRecModify.IsWindowEnabled())
	{
...
void CDlgFaceInfo::OnRecordNext() 
{
	// 如果处于修改状态，保存原来显示的FaceInfo
	if (!m_btnRecModify.IsWindowEnabled())
	{
...
void CDlgFaceInfo::OnRecordLast() 
{
	// 如果处于修改状态，保存原来显示的FaceInfo
	if (!m_btnRecModify.IsWindowEnabled())
	{
...
void CDlgFaceInfo::OnRecordModify() 
{
...
void CDlgFaceInfo::SetCtrlsStatus(BOOL bEnable)
{
	// 按界面中的控件顺序设置控件状态
	m_edtRecStaff.SetReadOnly(!bEnable);
	m_edtRecReview.SetReadOnly(!bEnable);
...
// 从ifo文件中读取掌子面信息
BOOL CDlgFaceInfo::ReadFaceInfoAndCanvas(CString strFileName, CTilCanvas* pCanvas)
{
	// 不管正面是否读取成功，掌子面数量均加1
	m_nFaceNum ++;
...
void CDlgFaceInfo::SetShowedFaceInfo(FACEINFOCANVAS& faceInfo, BOOL bCopyPrev)
{
	m_strTunName = faceInfo.strTunName;
	m_strPartIdxName = faceInfo.strPartIdxName;
	m_strRecStaff = faceInfo.strRecStaff;
	m_strRecReview = faceInfo.strRecReview;
...
// 	}
	...
	m_bFaceStatusZMBNZW = faceInfo.bFaceStatusZMJC;
...
void CDlgFaceInfo::GetShowedFaceInfo(FACEINFOCANVAS& faceInfo)
{
	UpdateData(TRUE);

	faceInfo.strTunName = m_strTunName;
	faceInfo.strPartIdxName = m_strPartIdxName;
	faceInfo.strRecStaff = m_strRecStaff;
	faceInfo.strRecReview = m_strRecReview;
...
// 为输入焦点时，设置数据显示格式
void CDlgFaceInfo::OnSetfocusEdtFaceMile() 
{
	// TODO: Add your control notification handler code here
	/*
...
// 失去焦点时，设置数据显示格式
void CDlgFaceInfo::OnKillfocusEdtFaceMile() 
{
	// TODO: Add your control notification handler code here
	/*
	UpdateData(TRUE);
...
void CDlgFaceInfo::OnCheckFaceStatusOther() 
{
	UpdateData(TRUE);
...
void CDlgFaceInfo::OnCheckExcaveStatusOther() 
{
	UpdateData(TRUE);
...
void CDlgFaceInfo::OnCheckRockStrengthOther() 
{
	UpdateData(TRUE);
...
void CDlgFaceInfo::OnCheckWeatheringOther() 
{
	UpdateData(TRUE);
...
void CDlgFaceInfo::OnCheckCrackWidthOther() 
{
	UpdateData(TRUE);
...
void CDlgFaceInfo::OnCheckCrackStatusOther() 
{
	UpdateData(TRUE);
...
// 获得区域宽度、高度、面积
void CDlgFaceInfo::GetPolyCaption(float& fWidth, float& fHeight, float& fArea)
{
...
void CDlgFaceInfo::OnRecordFromXml() 
{
	::CoInitialize(NULL);
...
// 		pFaceInfo->hb=(LPCSTR)pNode->Gettext();	
		...
		// 掌子面状态
		pNode=pDoc->selectSingleNode("//FaceStateName");
		if(pNode!=NULL)
		{
			strPara=(LPCSTR)pNode->Gettext();
			if(strPara=="稳定")
		...
		// 围岩风化程度
		pNode=pDoc->selectSingleNode("//RockWeatheringLevel");
		if(pNode!=NULL)
		{
			strPara=(LPCSTR)pNode->Gettext();
			if(strPara=="微风化")
		...
		// 单轴抗压强度
		pNode=pDoc->selectSingleNode("//CompressiveStrength");
		if(pNode!=NULL)
		{
		...
		// 毛开挖面状态
		pNode=pDoc->selectSingleNode("//InitialConstructStateName");
		if(pNode!=NULL)
		{
			strPara=(LPCSTR)pNode->Gettext();
			if(strPara=="自稳")
...
void CDlgFaceInfo::OnRecordToXml() 
{
	// TODO: Add your control notification handler code here
	::CoInitialize(NULL);
	...
	if(dlg.DoModal()==IDOK)
	{
		...
		// 读取里程信息
		pNode=pDoc->selectSingleNode("//TunnelFaceConstructInfo");
		IXMLDOMNodeListPtr pNodes=pNode->GetchildNodes();
		pNode=pNodes->Getitem(0); // Legend:dk1+111.1
		strPara=(LPCSTR)pNode->Gettext();
		if(strPara!=pFaceInfo->strFaceMile)
		{
			MessageBox("当前掌子面里程与xml文件中的里程不匹配,无法导入数据!","错误提示");
			return;
		}
		...
		if(pFaceInfo->bExcaveStatusZW)
			...
			pNode->Puttext("随时松弛、掉块");
		else if(pFaceInfo->bExcaveStatusJSZH)
			pNode->Puttext("自稳困难、要及时支护");
