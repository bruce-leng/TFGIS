...
#define CLASSVIEW 0
//IMPLEMENT_DYNAMIC(CGuiSolExplorer, CGuiControlBar)
...
{
	HTREEITEM hRoot = m_TreeSolExplorer.InsertItem (_T("隧道名称"), 0, 0);
	HTREEITEM hSrc = m_TreeSolExplorer.InsertItem (_T("工点名称"), 11, 12, hRoot);
...
void CGuiSolExplorer::InsertFileToSolExplorer(const CString& fileName)
{
	...
	if (hRoot != NULL)
	{
		if (hPrj == NULL)
		{
			hPrj = m_TreeSolExplorer.InsertItem(_T("工点名称"), 16, 16, hRoot);
		}
		if (hPrj != NULL)
		{
...
void CGuiSolExplorer::InsertProjToSolExplorer(CTilCanvasMgr* pProject)
{
	HTREEITEM hRoot = m_TreeSolExplorer.GetRootItem();
	HTREEITEM hPrj = m_TreeSolExplorer.GetChildItem(hRoot);
	if (hRoot != NULL)
	{
		...
		CString str; 
		str = get_file_name(pProject->GetPathName());
		if (!pProject->m_PrjCap.strTunName.IsEmpty())
		{
			// 设置隧道名称
			m_TreeSolExplorer.SetItemText(hRoot, _T(pProject->m_PrjCap.strTunName));

			// 设置洞室工点名称
			CString str;
			str = pProject->m_PrjCap.strPartName;
			if (!pProject->m_PrjCap.strPartIdx.IsEmpty())
				str = pProject->m_PrjCap.strPartIdx + "#" + str;
			hPrj = m_TreeSolExplorer.InsertItem(_T(str), 16, 16, hRoot);
		}
		else
		{
			m_TreeSolExplorer.SetItemText(hRoot, _T("隧道名称"));
			hPrj = m_TreeSolExplorer.InsertItem(_T("工点名称"), 16, 16, hRoot);
		}

/*
		if (pProject->m_pPrjName != NULL)
		{
			m_TreeSolExplorer.SetItemText(hRoot, _T(pProject->m_pPrjName));
			hPrj = m_TreeSolExplorer.InsertItem("洞室类型", 16, 16, hRoot);
		}
		else
		{
			hPrj = m_TreeSolExplorer.InsertItem(pProject->m_pPrjName->strPrjName,)
		}
		hPrj = m_TreeSolExplorer.InsertItem(pProject->m_pPrjName)
		hPrj = m_TreeSolExplorer.InsertItem(str, 16, 16, hRoot);*/

		int nSize = pProject->Size();
		for (int i = 0; i < nSize; i ++)
		{
			CString strPathName = pProject->GetCanvasByIdx(i)->GetImage()->GetPathName();
			strPathName = get_file_name(strPathName);
			m_TreeSolExplorer.InsertItem(strPathName, 7, 7, hPrj);
		}
	}
