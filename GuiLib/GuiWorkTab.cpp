...
void CGuiWorkTab::Drawtabs(CDC* dc)
{
	...
	if (!::IsWindow(m_toolBtn[0].GetSafeHwnd()))
	{
		...
		m_toolBtn[0].SetColor(GuiDrawLayer::GetRGBColorXP());	
		m_toolBtn[1].SetColor(GuiDrawLayer::GetRGBColorXP());	
	...
	{	
		...
		CPoint Coor[4];
			...
			if (!bIniciaArrastre)
			{
				...
				m_toolBtn[0].MoveWindow(m_rectTab.left+1,m_rectTab.top+1,szBtn.cx,m_rectTab.Height()-2);
				CSize szBtn1=m_toolBtn[1].GetSizeButton();
				m_toolBtn[1].MoveWindow(m_rectTab.left+szBtn.cx+1,m_rectTab.top+1,szBtn1.cx,m_rectTab.Height()-2);
			}
...
void CGuiWorkTab::OnSysColorChange() 
{
	...
	m_toolBtn[0].SetColor(GuiDrawLayer::GetRGBColorFace());	
	m_toolBtn[1].SetColor(GuiDrawLayer::GetRGBColorFace());	
	m_toolBtn[0].Invalidate();
    m_toolBtn[0].UpdateWindow();	
	m_toolBtn[1].Invalidate();
    m_toolBtn[1].UpdateWindow();
