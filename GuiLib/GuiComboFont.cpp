...
void CGuiComboFont::DrawCombo(enTypeShow enShow,CDC* pDC)
{
	...
	if (enShow==CM_NORMAL)
	{
		...
		m_rcClient.left-=1;
		m_rcClient.bottom-=1;
		pDC->Draw3dRect(&m_rcClient,GuiDrawLayer::GetRGBColorBTNHigh(),GuiDrawLayer::GetRGBColorBTNHigh());
		pDC->Draw3dRect(&m_rc,GuiDrawLayer::GetRGBColorBTNHigh(),GuiDrawLayer::GetRGBColorBTNHigh());
		m_rcClient.right += 1;
		m_rcClient.left+=1;
		m_rcClient.bottom+=1;
